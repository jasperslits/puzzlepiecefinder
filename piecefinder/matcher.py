...
class Matcher:
    ...
    async def processpiece(self,piece_file) -> None:
        ...
        for i in range(NUM_COLS * NUM_ROWS):
            res = await self.find_puzzle_piece_multi(f"{self.puzzle.pieces_dir}/piece_{i}.jpg",i)
            if res.score > 0:
                res.slice_index = i
    ...
    async def __copyoutput(self,found: str) -> None:
    ...
    async def find_puzzle_piece_multi(self, splitted: str, i: int) -> BlockDto:
        """Flip"""

        """Multi algo"""
        for algo in [Algorithm.TM,Algorithm.BF,Algorithm.SF]:
            self.alg = algo
            b = await self.find_puzzle_piece(splitted,i)
            if b.score > 0:
                return b
            continue
    ...
    async def find_puzzle_piece(self, splitted: str, i: int) -> BlockDto:
        ...
        for r in [False,True]:
            if r:
                piece_gray = cv2.rotate(piece_gray, cv2.ROTATE_180)
                print("  Rotated piece 180 degrees for matching.")
            else:
