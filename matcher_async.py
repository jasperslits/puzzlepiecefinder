...
async def slice_image(image_path: str, output_dir: str):
    """
...
async def copyoutput(found: str,alg: str,puzzle: str,piecename: str):
    rows = NUM_ROWS
    cols = NUM_COLS
...
async def find_puzzle_piece(puzzle_path: str, piece_path: str,i: int,alg: str,puzzlename: str,piecename: str) -> float:
    """
...
async def puzzlesetup(puzzlename: str):
...
async def processpiece(piece_file):
    ...
    await puzzlesetup(puzzle_name)
    pieces_dir = f"{puzzle_name}/splitted/"
    dir = os.listdir(pieces_dir)
    if len(dir) == 0:
        await slice_image(PUZZLE_FILE,pieces_dir)
    ...
    for i in range(NUM_COLS * NUM_ROWS):
        res = await find_puzzle_piece(f"{pieces_dir}piece_{i}.jpg",piece_file,i,alg,puzzle_name,piece_name)
        if res > 0:
            results[i] = res
    if len(results) == 0:
        ...
        await copyoutput(res,alg,puzzle_name,piece_name)

if __name__ == "__main__":
    asyncio.run(processpiece("input/t.png"))